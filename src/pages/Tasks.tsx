...
const Tasks = () => {
  ...
  const [savedSearches, setSavedSearches] = useState<Array<{
    id: string;
    name: string;
    filters: AdvancedSearchFiltersType;
  }>>([]);
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from('task_labels').select('label_name, label_color').order('label_name');
      if (error) throw error;
  ...
  const filteredTasks = React.useMemo(() => {
    return tasks.filter(task => {
      ...
      const matchesSearch = !advancedFilters.searchTerm || advancedFilters.searchFields.includes('title') && task.title?.toLowerCase().includes(searchTermLower) || advancedFilters.searchFields.includes('description') && task.description?.toLowerCase().includes(searchTermLower);
      ...
      const matchesStatus = advancedFilters.selectedStatus.length === 0 || advancedFilters.selectedStatus.includes(task.status || '');

      // Priority filtering (multi-select)
      const matchesPriority = advancedFilters.selectedPriority.length === 0 || advancedFilters.selectedPriority.includes(task.priority || '');

      // Label filtering
      const matchesLabels = advancedFilters.selectedLabels.length === 0 || task.labels && task.labels.some(label => advancedFilters.selectedLabels.includes(label.label_name));

      // Assignee filtering
      const matchesAssignee = advancedFilters.selectedAssignee === 'all' || advancedFilters.selectedAssignee === 'unassigned' && !task.assignee_id || task.assignee_id === advancedFilters.selectedAssignee;

      // Date range filtering
      const taskDueDate = task.end_date ? new Date(task.end_date) : null;
      const taskCreateDate = new Date(task.created_at);
      const matchesDueDateRange = (!advancedFilters.dueDateFrom || !taskDueDate || taskDueDate >= advancedFilters.dueDateFrom) && (!advancedFilters.dueDateTo || !taskDueDate || taskDueDate <= advancedFilters.dueDateTo);
      const matchesCreateDateRange = (!advancedFilters.createdDateFrom || taskCreateDate >= advancedFilters.createdDateFrom) && (!advancedFilters.createdDateTo || taskCreateDate <= advancedFilters.createdDateTo);
      ...
      return matchesSearch && matchesProject && matchesStatus && matchesPriority && matchesLabels && matchesAssignee && matchesDueDateRange && matchesCreateDateRange && quickFilterMatches;
    });
  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 relative overflow-hidden">
      ...
      <main className="container mx-auto py-8 px-4 relative z-10">
        ...
        <div className="text-center mb-12 space-y-6 animate-fade-in relative">
          ...
          }} className="flex items-center space-x-2 bg-white/30 backdrop-blur-sm hover:bg-white/50 transition-all duration-300 border border-white/20 text-slate-700 shadow-lg hover:shadow-xl">
          ...
          <div className="relative bg-white/20 backdrop-blur-sm rounded-2xl p-8 border border-white/30">
            ...
            <ExportDialog tasks={filteredTasks.map(task => ({
              id: task.id,
              title: task.title,
            ...
            <TaskForm projects={projects} teamMembers={teamMembers} availableTasks={tasks.map(t => ({
              id: t.id,
        ...
        title: t.title || 'Untitled',
        status: t.status || 'todo'
      }))} onSubmit={editingTask?.id === 0 ? handleCreateTask : handleUpdateTask} task={editingTask?.id === 0 ? null : editingTask} open={editDialogOpen} onOpenChange={open => {
        ...
        <Card className="mb-6 bg-white/70 backdrop-blur-xl border border-white/20">
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button className="group text-lg px-10 py-4 bg-gradient-to-r from-primary to-blue-600 hover:from-blue-600 hover:to-primary transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl font-semibold relative overflow-hidden" size="sm" onClick={() => setShowAdvancedFilters(!showAdvancedFilters)}>
                <span className="relative z-10 flex items-center">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
              <div className="relative">
                ...
                <Input placeholder="Search tasks..." value={advancedFilters.searchTerm} onChange={e => setAdvancedFilters(prev => ({
                ...prev,
                searchTerm: e.target.value
              }))} className="pl-10" />
              </div>
              
              <Select value={advancedFilters.selectedProject} onValueChange={value => setAdvancedFilters(prev => ({
              ...prev,
              ...
              <Popover>
                ...
                <PopoverContent className="w-64 p-3">
                  <div className="space-y-2">
                    {['todo', 'in_progress', 'review', 'completed', 'blocked'].map(status => <div key={status} className="flex items-center space-x-2">
                        <Checkbox id={status} checked={advancedFilters.selectedStatus.includes(status)} onCheckedChange={checked => {
                      const newStatus = checked ? [...advancedFilters.selectedStatus, status] : advancedFilters.selectedStatus.filter(s => s !== status);
              ...
              <Popover>
                ...
                <PopoverContent className="w-64 p-3">
                  <div className="space-y-2">
                    {['low', 'medium', 'high', 'critical'].map(priority => <div key={priority} className="flex items-center space-x-2">
                        <Checkbox id={priority} checked={advancedFilters.selectedPriority.includes(priority)} onCheckedChange={checked => {
                      const newPriority = checked ? [...advancedFilters.selectedPriority, priority] : advancedFilters.selectedPriority.filter(p => p !== priority);
                      setAdvancedFilters(prev => ({
                        ...prev,
                ...
                <PopoverContent className="w-64 p-0">
                  <div className="p-3">
                    ...
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {allLabels.length === 0 ? <p className="text-sm text-muted-foreground">No labels found</p> : allLabels.map(label => <div key={label.label_name} className="flex items-center space-x-2">
                            <Checkbox id={label.label_name} checked={advancedFilters.selectedLabels.includes(label.label_name)} onCheckedChange={checked => {
                        const newLabels = checked ? [...advancedFilters.selectedLabels, label.label_name] : advancedFilters.selectedLabels.filter(l => l !== label.label_name);
                        setAdvancedFilters(prev => ({
                          ...prev,
                          selectedLabels: newLabels
                        }));
                      }} />
                            <label htmlFor={label.label_name} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 flex items-center gap-2">
              ...
              <Button className="group text-lg px-10 py-4 bg-gradient-to-r from-primary to-blue-600 hover:from-blue-600 hover:to-primary transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl font-semibold relative overflow-hidden" onClick={() => setAdvancedFilters({
              searchTerm: '',
              searchFields: ['title', 'description'],
              selectedProject: 'all',
              selectedStatus: [],
        ...
        {/* Advanced Filters */}
        {showAdvancedFilters && <AdvancedSearchFilters filters={advancedFilters} onFiltersChange={setAdvancedFilters} projects={projects} teamMembers={teamMembers} allLabels={allLabels} savedSearches={savedSearches} onSaveSearch={handleSaveSearch} onLoadSearch={handleLoadSearch} onDeleteSearch={handleDeleteSearch} />}

        {/* Task Views */}
        <Tabs defaultValue="list" className="w-full">
          <TabsList className="grid w-full grid-cols-3 lg:w-[400px] bg-white/70 backdrop-blur-xl border border-white/20 px-0 mx-0 my-[15px]">
          ...
          <TabsContent value="list" className="mt-6">
            {tasksLoading ? <div className="text-center py-8">Loading tasks...</div> : filteredTasks.length === 0 ? <Card>
                <CardContent className="text-center py-8">
                  <p className="text-muted-foreground">No tasks found</p>
                </CardContent>
              </Card> : <div className="grid gap-4">
                 {filteredTasks.map(task => <TaskItem key={task.id} task={task} onEdit={setEditingTask} onDelete={handleDeleteTask} onStatusChange={handleStatusChange} onView={handleTaskView} />)}
              </div>}
          </TabsContent>
          ...
          <TabsContent value="kanban" className="mt-6">
            <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragOver={handleDragOver} onDragEnd={handleDragEnd}>
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 min-h-[600px]">
                {statusColumns.map(column => <KanbanColumn key={column.id} id={column.id} title={column.label} tasks={currentTasksByStatus[column.id as keyof typeof currentTasksByStatus]} color={column.color} onEditTask={setEditingTask} />)}
              </div>
              
              <DragOverlay>
                {activeTask ? <KanbanTaskCard task={activeTask} onEdit={() => {}} /> : null}
              </DragOverlay>
            </DndContext>
          </TabsContent>
        ...
        {/* Task Details Dialog */}
        <TaskDetailsDialog task={selectedTask} open={taskDetailsOpen} onOpenChange={setTaskDetailsOpen} onTaskUpdate={handleTaskDetailsUpdate} />
      </main>
    </div>;
};
